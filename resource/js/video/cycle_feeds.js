...
{
    var new_info_ = W('#feeds_num_');
    
    if( !window.XTimer ) return;
    if( !new_info_.length ) return;
    
    window.NEW_FEED_API = window.NEW_FEED_API || '/circle/newFeedsCount';
    ...
    (
        {
            ...
            , "doneCallback":
                function( $second )
                {
                    ...
                    (
                        window.NEW_FEED_API
                        , { 'id': window.CIRCLE_ID, "tm": window.PARAM_LASTTIME, "rnd": Math.random() }
                        , function( $d )
                        {   
                            CHECK_NEW_INFO();
                            ...
                            {
                                new_info_.html( $d.data + '(新)' );
                                new_info_.attr( 'onclick', 'onclick="reload_page()' );
                                new_info_.style.css('cursor', 'pointer');
                            }
                        }
...
function LOADING_MORE_INFO()
{
    ...
    window.MORE_FEED_API = window.MORE_FEED_API || '/circle/morefeed'; 
