/**
 * @fileoverview Url class
 * @author:{@link mailto:greengnn@gmail.com JianbinZhang}
 * @last-modified : 2012-4-06
 */

(function(){
	var browsers = QW.Browser,
		O = QW.ObjectH,
		A = QW.ArrayH,
		_isArray = O.isArray,
    ...
    function hashRoute(route, handler){
        var rule = _isArray(route) ? route[0] : route;
    ...
    function param(obj){
        obj = _isArray(obj) ? obj.slice() : [obj];
        var s = obj.shift(), o = [];
        obj = A.filter(function(a){ return a !== ""; });
        for (var k in s) {
            if (k) {
                o.push(k + '=' + s[k]);
            }
    ...
    function URLkit(win, opt){
        ...
        this.handler = function(){
            var succ;
            A.forEach(self._route_config,function(args){
